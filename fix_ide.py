...
pattern = r"const fetchAIResponse = async \(chatHistory\) => \{.*?(?:await streamAIResponse\(newMessages\);\s*\n\s*\};\s*)"

new_code = """  const fetchAIResponse = async (chatHistory) => {
...
print(f"Replaced {len(content)} with {len(new_content)} characters.")